...
import idl from "@zwap/solana/idl.json";
...
export class Relayer {
	...
	constructor(config: RelayerConfig) {
		...
		// Create a dummy wallet for the program (read-only)
		const dummyWallet = {
	...
	private async processDeposit(
		...
		try {
			...
			const amountNumber = typeof event.amount === 'object' && 'toNumber' in event.amount
				? (event.amount as BN).toNumber()
				: Number(event.amount);
			
			const zecAmount = this.convertToZec(
				event.asset,
				amountNumber,
			);
